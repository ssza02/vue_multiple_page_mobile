//js日历
var calendar = function(calendarId,now){
    //日历对象不存在，返回日历
    ...
    var thismonth =  new Date(now).getMonth()+1;
    //生成日历的函数
    c.getcalendar = function(year,month){
        ...
        //星期是从0开始到6结束的，小于等于上个月最大星期的日期则是上个月的日期
            if(g <= lastday){
                str +='<td class="Tr">'+'</td>';//+ remainlastdate++ 删除了这段是为了不显示上个月的时间。
            }else{
                //高亮显示当前日期
        ...
        for(var i=1;i<=5;i++){
            ...
            for(var j=0;j<7;j++){
                if(remainlastday <= Maxdate){
                    ...
                    str +='<td class="Tr">'+'</td>';//+ nextMonthday++ 删除了这段是为了不显示下个月的时间
                }
            }
            str += '</tr>';
        }
    ...
    c.init = function(){
        ...
        loadStyles(".calendarDiv {width:100%;text-align:center;}\
            ...
            .box{margin-left:15px;margin-right:10px;text-align:center;width:335px;height:22px;}\
            ...
            .redTr{color:#D6D6D6;text-align:center;}\
            .blueTr{color:#D6D6D6;text-align:center;};");
        }
